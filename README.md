<div align="center" style="background-color:#0d1117; padding: 20px; border-radius: 10px;">
  
  <h1 style="color:#58a6ff;">👋 Hi, I'm <strong>Dilshan Wickramasinghe</strong></h1>
  <h3 style="color:#c9d1d9;">Software Engineering Intern • Full-Stack Developer • Tech Enthusiast</h3>

  <img src="https://github-readme-stats.vercel.app/api?username=Dilshan-Wickramasinghe&show_icons=true&theme=tokyonight&hide_border=true" width="55%" />
  
</div>

---

<h2 style="color:#58a6ff;">🔧 Tech Stack</h2>

<h3 style="color:#c9d1d9;">Frontend</h3>
<ul style="color:#8b949e;">
  <li>HTML, CSS, JavaScript</li>
  <li>React.js, Bootstrap</li>
</ul>

<h3 style="color:#c9d1d9;">Backend</h3>
<ul style="color:#8b949e;">
  <li>Java (Spring Boot), Python, PHP</li>
  <li>REST APIs</li>
</ul>

<h3 style="color:#c9d1d9;">Databases</h3>
<ul style="color:#8b949e;">
  <li>MySQL, PostgreSQL, MongoDB</li>
</ul>

<h3 style="color:#c9d1d9;">DevOps & Cloud</h3>
<ul style="color:#8b949e;">
  <li>Git, GitHub</li>
  <li>Docker (basic), AWS (basic)</li>
</ul>

<h3 style="color:#c9d1d9;">Tools</h3>
<ul style="color:#8b949e;">
  <li>Postman, Figma, IntelliJ IDEA, VS Code, JIRA</li>
  <li>Cisco Packet Tracer</li>
</ul>

---

<h2 style="color:#58a6ff;">🧪 Projects</h2>

<h3 style="color:#c9d1d9;">📘 Brahmi Character Recognition (Ongoing)</h3>
<p style="color:#8b949e;">
<strong>Python, TensorFlow, PyTorch, OpenCV, Flask</strong><br>
Building DL models for Brahmi character recognition & historical period classification.
</p>

<h3 style="color:#c9d1d9;">🎵 Music Streaming Application</h3>
<p style="color:#8b949e;">
<strong>HTML, CSS, JS, JSP, Java</strong><br>
Music streaming desktop web app with playlists and playback.
</p>

<h3 style="color:#c9d1d9;">💊 Pharmacy Management System</h3>
<p style="color:#8b949e;">
<strong>PHP, MySQL, JS, Bootstrap</strong><br>
Inventory, stock management, prescription ordering & RBAC.
</p>

<h3 style="color:#c9d1d9;">🩺 Telemedicine App (Ongoing)</h3>
<p style="color:#8b949e;">
<strong>Spring Boot, React.js, JWT, PostgreSQL, Thymeleaf</strong><br>
Secure authentication, appointments, consultations & API integrations.
</p>

---

<h2 style="color:#58a6ff;">📚 Certifications</h2>
<ul style="color:#8b949e;">
  <li>Java Foundations – JetBrains</li>
  <li>AWS Cloud Essentials – AWS</li>
  <li>JavaFX GUI Development – LinkedIn</li>
  <li>Web Design for Beginners – UoM</li>
  <li>Python for Beginners – UoM</li>
  <li>Docker Foundations – Docker, Inc</li>
  <li>Networking Essentials – Cisco</li>
  <li>AI/ML Engineer – Stage 1 – SLIIT</li>
</ul>

---

<h2 style="color:#58a6ff;">🤝 Volunteering</h2>
<ul style="color:#8b949e;">
  <li>Leo Club • UWU</li>
  <li>IEEE Computer Society • UWU</li>
</ul>

---

<h2 style="color:#58a6ff;">📫 Connect With Me</h2>

<p style="color:#8b949e;">
📧 <strong>Email:</strong> <a href="mailto:dilshanwickramasinghe007@gmail.com" style="color:#58a6ff;">dilshanwickramasinghe007@gmail.com</a><br>
🔗 <strong>LinkedIn:</strong> <a href="https://www.linkedin.com/in/dilshan-wick/" style="color:#58a6ff;">linkedin.com/in/dilshan-wick</a><br>
💻 <strong>GitHub:</strong> <a href="https://github.com/Dilshan-Wickramasinghe" style="color:#58a6ff;">github.com/Dilshan-Wickramasinghe</a><br>
✍️ <strong>Medium:</strong> <a href="https://medium.com/@dilshantharindu9952" style="color:#58a6ff;">medium.com/@dilshantharindu9952</a>
</p>

---

<div align="center" style="padding: 20px;">
  <h3 style="color:#58a6ff;">✨ Thanks for visiting my profile!</h3>
  <p style="color:#8b949e;">Feel free to ⭐ my repositories if you find them useful.</p>
</div>

